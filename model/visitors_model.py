...
class Event(db.Model):
    ...
    partnership = db.relationship('Partnership', foreign_key=[id_partnership], cascade='merge')
    hour_since = db.Column(db.DateTime, nullable=False)
    hour_until = db.Column(db.DateTime, nullable=False)
    id_user = db.Column(db.Integer, db.ForeignKey('users.id'))
    user_creator = db.relationship('User', foreign_key=[id_user], cascade='merge')
...
class VisitorPerEvent(db.Model):
    ...
    visitor = db.relationship('Visitor', foreign_key=[id_visitor], cascade='merge')
    id_event = db.Column(db.Integer, db.ForeignKey('events.id'))
    event = db.relationship('Event', foreign_key=[id_event], cascade='merge')
